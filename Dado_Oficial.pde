...
ArrayList<Integer> valoresDados = new ArrayList<Integer>();
...
void keyPressed() {
  if (key == ' ') {
    ...
    valoresDados.add(posicionAleatoria + 1);
    dibujarImagen = true;
  } else if (key == 'm') { 
    ...
    valoresDados.clear();
    println("Lista borrada (ultimo valor obtenido)."); // y B para borrar lista hasta antes del ultimo :D
    for (int i = 0; i < dadoMostrado.length; i++){
      dadoMostrado[i] = false;
  }
}
...
void dibujarDados() {
  ...
  int startIndex = max(valoresDados.size() - 20, 0); //Muestra hasta 20 elemtos :)
  for (int i = startIndex; i < valoresDados.size(); i++) {
    println(valoresDados.get(i));
  }
}
